...
export interface MisuEditAnnotation {
  type?: "edit_data";
  kind?: "edit";
  edit_id: string;
  document_id: string;
...
export type AssistantEvent =
  ...
  | {
        ...
        version_number?: number | null;
        download_url: string;
        annotations: MisuEditAnnotation[];
        error?: string;
        isStreaming?: boolean;
    }
 ...
 */
export interface MisuCitationAnnotation {
  type: "citation_data";
  ref: number;
  doc_id: string;
  document_id: string;
...
export function formatCitationPage(a: MisuCitationAnnotation): string {
  if (typeof a.page === "string") return `Page ${a.page}`;
  return `Page ${a.page}`;
}

/** Produce a reader-friendly version of the quote (replaces [[PAGE_BREAK]] with "..."). */
export function displayCitationQuote(a: MisuCitationAnnotation): string {
...
export interface MisuChatDetailOut {
  chat: MisuChat;
  messages: MisuMessage[];
}
